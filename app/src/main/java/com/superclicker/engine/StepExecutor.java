...
public class StepExecutor {
    ...
    public boolean executeNumberCompare(Step step) {
        try {
            double v1 = Double.parseDouble(step.compareValue1);
            double v2 = Double.parseDouble(step.compareValue2);
            return compare(v1, v2, step.compareOperator);
        } catch (Exception e) { return false; }
    }
    ...
    public boolean executeTimeCompare(Step step) {
        try {
            long target = Long.parseLong(step.compareValue2);
            return compare(System.currentTimeMillis(), target, step.compareOperator);
        } catch (Exception e) { return false; }
    }
    ...
    public void executeCounter(Step step, Map<String, Integer> counters) {
        ...
        switch (step.counterOp != null ? step.counterOp : "set") {
            case "set": current = step.counterValue; break;
            case "add": current += step.counterValue; break;
            case "sub": current -= step.counterValue; break;
        }
    ...
    private void await(CountDownLatch latch, long ms) {
        try { latch.await(ms, TimeUnit.MILLISECONDS); } catch (InterruptedException ignored) {}
    }
}
