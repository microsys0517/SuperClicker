...
public class Step {
    ...
    public Step copy() {
        ...
        s.type = this.type;
        s.label = this.label;
        s.waitBefore = this.waitBefore;
        s.waitAfter = this.waitAfter;
        s.retryCount = this.retryCount;
        s.retryDelay = this.retryDelay;
        s.enabled = this.enabled;
        s.x1 = this.x1; s.y1 = this.y1;
        s.x2 = this.x2; s.y2 = this.y2;
        s.duration = this.duration;
        s.multiPoints = this.multiPoints;
        s.matchImagePath = this.matchImagePath;
        s.matchColor = this.matchColor;
        s.colorTolerance = this.colorTolerance;
        s.matchText = this.matchText;
        s.regionJson = this.regionJson;
        s.compareValue1 = this.compareValue1;
        s.compareValue2 = this.compareValue2;
        s.compareOperator = this.compareOperator;
        s.counterName = this.counterName;
        s.counterValue = this.counterValue;
        s.counterOp = this.counterOp;
        s.inputText = this.inputText;
        s.jumpTarget = this.jumpTarget;
        s.jumpCondition = this.jumpCondition;
        s.subScriptId = this.subScriptId;
        s.errorRule = this.errorRule;
        return s;
    }
}
