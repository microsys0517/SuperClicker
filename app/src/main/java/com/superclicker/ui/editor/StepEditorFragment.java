...
public class StepEditorFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.btn_back).setOnClickListener(v -> { if (getActivity() != null) getActivity().getSupportFragmentManager().popBackStack(); });
        ...
        view.findViewById(R.id.btn_delete_step).setOnClickListener(v -> deleteSelectedSteps());
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        adapter = new StepAdapter(step -> showStepConfig(step));
    ...
    private void refreshSteps() {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            ...
            if (getActivity() != null) getActivity().runOnUiThread(() -> { adapter.setData(steps); tvStepCount.setText(steps.size() + " 步"); });
        });
    }
    private void showAddStepDialog() {
        ...
        new AddStepDialog(requireContext(), type -> {
            Step step = new Step(); step.scriptId = currentScript.id; step.type = type; step.label = type.label;
            Executors.newSingleThreadExecutor().execute(() -> {
                step.order = db.stepDao().getMaxOrder(currentScript.id) + 1;
                db.stepDao().insert(step);
                if (getActivity() != null) getActivity().runOnUiThread(this::refreshSteps);
            });
        }).show();
    }
    ...
    private void deleteSelectedSteps() {
        ...
        new AlertDialog.Builder(requireContext()).setTitle("删除").setMessage("删除 " + sel.size() + " 个步骤？")
            .setPositiveButton("删除", (d, w) -> Executors.newSingleThreadExecutor().execute(() -> {
                List<Long> ids = new ArrayList<>(); for (Step s : sel) ids.add(s.id);
                db.stepDao().deleteByIds(ids);
                if (getActivity() != null) getActivity().runOnUiThread(() -> { adapter.clearSelection(); refreshSteps(); });
    ...
    private void executeScript() {
        ...
        Executors.newSingleThreadExecutor().execute(() -> {
            List<Step> steps = db.stepDao().getByScriptIdSync(currentScript.id);
            if (steps.isEmpty()) { if (getActivity() != null) getActivity().runOnUiThread(() -> Toast.makeText(requireContext(), "脚本为空", Toast.LENGTH_SHORT).show()); return; }
            if (getActivity() != null) getActivity().runOnUiThread(() -> { engine = new ScriptEngine(requireContext()); engine.start(currentScript, steps); });
        });
    }
}
