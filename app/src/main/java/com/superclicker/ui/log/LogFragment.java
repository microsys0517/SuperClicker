...
public class LogFragment extends Fragment implements Logger.LogListener {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        view.findViewById(R.id.btn_clear_log).setOnClickListener(v -> { Logger.clear(); adapter.setData(new ArrayList<String>()); });
        return view;
    }
    @Override
    ...
    @Override
    public void onNewLog(String msg) { if (adapter != null) { adapter.appendLog(msg); rvLogs.scrollToPosition(adapter.getItemCount() - 1); } }
    static class LogAdapter extends RecyclerView.Adapter<LogAdapter.VH> {
        ...
        @NonNull @Override public VH onCreateViewHolder(@NonNull ViewGroup p, int v) { TextView tv = new TextView(p.getContext()); tv.setTextSize(11); tv.setPadding(4, 2, 4, 2); tv.setTypeface(Typeface.MONOSPACE); return new VH(tv); }
        @Override public void onBindViewHolder(@NonNull VH h, int pos) {
            String line = data.get(pos); ((TextView) h.itemView).setText(line);
